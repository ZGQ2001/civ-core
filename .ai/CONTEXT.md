...
## 当前焦点（2026-09-10）— 本机运行与测试基线

用户授权先跑通现有功能，建立可信测试基线。本机已确认 .NET 9.0.318、Python 3.12、uv、Node 24、Rust MSVC 和 C++ Build Tools 可用，依赖已还原。

- C# 测试入口修正为显式测试项目，并初始化规范库：264 通过、1 项真实数据集成测试跳过。此前直接跑测试因规范库缺失有 28 项失败。
- Python 226 项、MCP 7 项、Rust 2 项测试通过；前端构建、lint 和格式检查通过。
- 修复 Windows MCP 测试的固定等待、报告页面 Hook 依赖，以及 Vite 监听 Rust 编译产物导致的 `EBUSY` 启动故障。
- 新增 `mcp/scripts/pipeline-smoke.mjs` 和配套夹具：通过真实 MCP/sidecar 调用生成锚杆、防火涂层、曲线与混合报告，核对 5 张表、3 张嵌图和字段替换。
- CI 补 MCP 检查与完整链路产物、Rust 单元测试、C# 测试记录；开发文档同步正确命令。
- 详细结果及复现方式见 `docs/baseline-2026-09-10.md`。使用合成样例；真实项目验收、Word 打印分页和 T6 打包仍未完成。本轮不提交、不推送。
- 桌面窗口 `筑核 civ-core` 已启动，两个 sidecar 已拉起；最终样例在 `logs/baseline/sample-final/`。并行首次编译时曾发生一次 Python 启动超时，复跑通过，未修改超时阈值。

---

## 历史焦点（2026-06-03 续）— 换价值主张(D)去黑盒 + 7 目标推进（分支 `claude/magical-cori-a9T8w`，PR #22 draft）
