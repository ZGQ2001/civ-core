...
**2026-05-20 UI 技术栈转型**：Qt + qfluentwidgets 视觉天花板过不去，转 Tauri + React + Python sidecar 路线。旧 `ui/` `apps/bootstrap.py` `pyside6` 已全删；业务底座 `domain/ core/ infra_io/` 完全保留。

---

## 工作流

1. 会话开始 `git add . && git commit -m "chore: 会话开始检查点"` → 读 `docs/dev_journal/PROGRESS.md` **顶部摘要** → 报告当前状态/待办/下一步 → 确认后动手
2. 单步骤 `git add . && git commit -m "feat: 完成 xxx"`
3. 新增/修改文件 → 先写 pytest 拦边界 → 写实现 → `uv run --frozen ruff check .` → `uv run --frozen pytest` → `uv run --frozen python scripts/healthcheck.py`
4. 会话结束 → 更新 `PROGRESS.md`
...
| `src/civ_core/api/` | JSON-RPC server（stdin/stdout，Tauri sidecar） | `server.py` + `handlers/{workspace,files,...}.py` |
...
| `frontend/` | Vite + React + TS + Tailwind v4 + codicons | UI 全在此 |
| `frontend/src-tauri/` | Tauri 2 主进程（Rust） | spawn Python sidecar，`rpc_call` 转发 |
...
依赖方向：`frontend → Tauri rpc_call → api/server.py → core/ → infra_io/ → domain/`。禁反向 import。
...
| 模块 | 职责 |
|---|---|
| `api/server.py` | `Dispatcher` JSON-RPC 路由 + `serve()` stdin/stdout 行循环 |
| `api/__main__.py` | `python -m civ_core.api` 入口；自己挂"文件 + stderr"logger（绝不污染 stdout 协议流） |
| `api/handlers/workspace.py` | `workspace.{last,set,clear,create_standard}`；存到 `~/.civ-core/workspace.json` |
| `api/handlers/files.py` | `files.{list_dir,exists}`；默认隐藏 `.civ-core` 和点开头 |
| `frontend/src-tauri/src/lib.rs` | Tauri 启动 + setup 里 spawn sidecar + `rpc_call` command |
| `frontend/src-tauri/src/sidecar.rs` | `PythonSidecar`：ChildStdin/Stdout Mutex 包，串行 JSON-RPC |
| `frontend/src/App.tsx` | 顶层 layout：TitleBar + ActivityBar + SideBar + EditorArea + StatusBar |
| `frontend/src/lib/rpc.ts` | `invoke('rpc_call', ...)` 包装 |
| `core/plot_curves.py` | Excel → PlotJob → 批量出图（SVG） |
| `core/calc_functions.py` | INSP-001/002/003 计算函数（里氏 / 钻芯 / 回弹） |
| `infra_io/standards_db.py` | SQLite 通用查表层（规范系数表） |
| `infra_io/workspace_scaffold.py` | 标准项目骨架生成（委托方资料/数据/报告/模板/.civ-core） |
...
| 跨语言协议 | JSON-RPC 2.0 over stdin/stdout，行协议 |
| Excel 主路径 | openpyxl-only（`excel_reader.py`），禁主流程 pandas |
| Excel 辅助 | pandas 延迟 import |
| Word | docxtpl + python-docx；COM 仅域刷新/目录/转 PDF/精修，集中在 `utils/word_com.py` |
| 图片 | `.svg`（前端通过 RPC 拿 svg 字节渲染） |
...
cd frontend && npm run tauri:build            # 安装包（T6 PyInstaller 配好才完整）
```
...
| 在 api server 里 print 到 stdout | stdout 是协议流；日志走 stderr 或文件 |
