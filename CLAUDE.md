...
uv run python -m civ_core.main            # 启动旧 Qt GUI（T7 阶段会删）
uv run python -m civ_core.api             # 启动 JSON-RPC server（Tauri sidecar 用，stdin/stdout 协议）
...
# Tauri + 前端（新主 UI，2026-05-20 起转型中）
bash run.sh                               # 一键启动开发模式（Tauri + Vite + Python sidecar）
cd frontend && npm run tauri:dev          # 同上，等价命令
cd frontend && npm run build              # 仅 build 前端
cd frontend/src-tauri && cargo check      # 仅检查 Rust（不打包）
cd frontend && npm run tauri:build        # 打安装包（T6 接 PyInstaller 后才完整可用）

# DEV_MODE：config.toml → [dev] enabled = true，用户预设改读 tests/fixtures/presets/
```

### 中国镜像（下载依赖必走，直连国外源在国内常超时）

| 工具链 | 镜像配置位置 | 镜像源 |
|---|---|---|
| Cargo（Rust crates） | `frontend/src-tauri/.cargo/config.toml`（项目级，已配） | 字节 `sparse+https://rsproxy.cn/index/` |
| rustup（Rust toolchain） | 用户 shell 环境变量 | `RUSTUP_DIST_SERVER=https://rsproxy.cn` + `RUSTUP_UPDATE_ROOT=https://rsproxy.cn/rustup` |
| npm（前端包） | 暂未配（小包够快）；需要时项目级 `.npmrc` 加 `registry=https://registry.npmmirror.com` | 淘宝 npmmirror.com |
| pip / uv（Python） | 暂未配；需要时 `UV_INDEX_URL=https://mirrors.aliyun.com/pypi/simple/` | 阿里云 |

新增 cargo 依赖前确认镜像生效；调试用 `cargo build -v` 看实际下载源。
