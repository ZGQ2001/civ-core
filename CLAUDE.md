...
**新想法 / 新重构：先找成熟方案，别先想「我怎么做」。** 业界有现成的标准做法 / 设计模式 / 库就照它走（说清是哪个）；确认没有或不合用，再设计定制方案，并讲明为什么不用现成的。
...
token 烧在「每轮重发的固定底座 + 累积的历史和工具输出」，跟打多少字无关。编码时：

- 大范围搜索（找文件/用法/schema）委派 Explore/子 agent，只取结论，别把翻过的文件灌进主上下文。
- 读文件给 `file:line` 或函数名，用 Read 的 offset/limit，别整个大文件读进来。
- 测试/构建日志别整屏贴回，grep 关键行或重定向到文件再看。
- 用户侧杠杆（一任务一会话 / `/clear` / `/compact` / `/context` / 关无关 MCP / routine 降模型）见 `.ai/RULES.md`。
