...
| `old_code/`                 | 旧代码（02_Core / 04_Config 等）  | 参考，待迁移后删除                                   |
...
## 已知技术债（详见 PROGRESS.md）

- **P1.5**：曲线点悬停 → 表格联动（需 PNG 坐标反向映射）；`highlight_row` 图上突出标记未实装
- **P2**：`old_code/02_Core/` 未迁移（41 个 pyright 报错全在这里）；旧 UI 组件 `preset_list.py` / `preset_form_panel.py` / `preview_pane.py` 已不被引用待删；`tests/test_cross_ref_fix.py` 在 `addopts` 默认 ignore
- **P3**：新工具接入 `word2pdf` / `auto_filler` / `bracket_normalize`
