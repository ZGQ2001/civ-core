...
/**
 * 检测类型 —— 报告填充页第一步「勾选」要出哪些检测（可多选）：
 *   anchor  : 锚杆抗拔（纯锚杆走 anchor.run / report.run_from_result，按数据来源 raw/result）
 *   coating : 防火涂层（report.assemble，模板含 {{表格:防火涂层}}）
 * 勾多个 → 组装到一份报告（report.assemble，按勾选拼 sections，模板含对应占位符）。
 * 不再有写死的「multi」——多选天然表达任意组合。以后加钻芯/回弹 = 往这里加一项，多选框自动支持。
 */
export const DETECTION_TYPES = [
  { id: 'anchor', label: '锚杆抗拔' },
  { id: 'coating', label: '防火涂层' },
] as const;
export type DetectionType = (typeof DETECTION_TYPES)[number]['id'];
