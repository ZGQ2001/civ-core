...
export const coatingExpandTemplate: ToolDef = {
  ...
  description:
    ...
    "布局按 标准×涂层类型：国标薄/超薄型→「测点数据-<类型>-膨胀型」5 处×3 点（处号1~5，列头 测点1/测点2/测点3，参照 GB/T 50621 §12 防腐口径）；" +
...
export const coatingReport: ToolDef = {
  ...
  description:
    "防火涂层一键 Word 报告：读 coating_run 已算好的结果 xlsx（不重算）+ 把数据表填进 docx 薄壳模板 → 出 .docx。" +
    ...
    "前置：先 coating_run 出结果 xlsx。",
  inputSchema: {
    result_xlsx: z.string().describe("coating_run 产出的结果 Excel 绝对路径（含机读结果 sheet）"),
    ...
    output_docx: z
      .string()
      .optional()
      .describe("输出 docx 绝对路径。缺省在结果 xlsx 同目录写 '<原名>_防火涂层_报告.docx'"),
