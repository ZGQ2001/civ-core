/**
 * coating.* —— 防火涂层厚度（GB 50205-2020 §13.4.3 厚涂型验收）装配线 4 tool。
 *
 * 流程（构件清单驱动，用户/agent 几乎只填数字）：
 *   1. coating_generate_template 出「类型预设 + 构件清单」模板
 *   2. （填「构件清单」：一构件一行，构件位置/类型/长度或截面数/设计厚度）
 *   3. coating_expand_template 展开成「测点数据-<类型>」面名网格
 *   4. （在网格里填实测数字）
 *   5. coating_run 跑判定，每批出「<批>-数据分析」宽表
 *
 * 涂层类型按设计厚度自动分级（≥7厚/3~7薄/≤3超薄）；本轮只厚型出合格/不合格，薄/超薄判定待接入。
...
export const coatingExpandTemplate: ToolDef = {
  ...
  description:
    "读「构件清单」展开成「测点数据-<类型>」面名网格（每构件铺 N 截面行，测点列留空待填数字）。" +
    "解析规则：构件类型空则从构件位置名识别（含「梁/柱」）；截面数空则按 ⌈长度/间距⌉（国标3m/北京地标1m）；" +
    "设计厚度空则用类型预设默认（个别构件不同时在构件清单填覆盖）。" +
...
export const coatingRun: ToolDef = {
  ...
  description:
    ...
    "厚型判定（GB 50205 §13.4.3）：≥80% 测点 ≥ 设计厚度 且 最薄处 ≥ 设计 × 85%；薄型/超薄型本轮判定待接入。" +
    "返回 {batches, members_total, members_qualified, members_pending, output}。" +
