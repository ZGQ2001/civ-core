...
为什么把 CLI 和 UI 合一个 main：
  • 与 pyproject.toml [project.scripts] 入口 `civ-core` 一致，避免双入口
  • CLAUDE.md v2.3 第一阶段验收命令固定形如：
        uv run python -m civ_core.main --tool plot_curves --input ...
...
# 把 src/ 加入 path，确保 civ_core 包可被子进程 / `python script.py` 形式找到
...
def _run_cli(args: argparse.Namespace) -> int:
    ...
    try:
        from civ_core.configs.loader import ConfigError, load_config
        from civ_core.utils.logger import setup_from_config
