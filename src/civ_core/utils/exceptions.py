...
所有自定义异常顶层基类是 CivCoreError，便于 main.py / UI 层统一捕获兜底。
...
class CivCoreError(Exception):
    """全部业务/基础设施异常的顶层基类。

    禁止业务代码直接 `raise CivCoreError(...)` —— 必须落到下面 4 个子档之一。
...
# 1. 配置层
# ══════════════════════════════════════════════════════════════════
class ConfigError(CivCoreError):
...
# 3. 业务层
# ══════════════════════════════════════════════════════════════════
class BusinessError(CivCoreError):
    """业务规则不满足，与基础设施无关。"""
...
# 4. 基础设施层
# ══════════════════════════════════════════════════════════════════
class InfraIOError(CivCoreError):
    """文件 / 进程 / COM 等基础设施失败。"""
...
# ══════════════════════════════════════════════════════════════════
def format_for_log(exc: CivCoreError) -> str:
